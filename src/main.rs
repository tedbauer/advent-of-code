...
fn solution2(input: Lines<StdinLock<'_>>) -> Result<String> {
...
fn solution1(input: Lines<StdinLock<'_>>) -> Result<String> {
...
fn build_left_right(input: Lines<StdinLock<'_>>) -> Result<(Vec<i32>, Vec<i32>)> {
...
fn main() {
    ...
    let res = solution2(input).unwrap();
    println!("{res}");
}
